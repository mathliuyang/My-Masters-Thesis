\chapter[NFSWEs的能量和质量守恒方法]{NFSWEs的能量和质量守恒方法}
...
\begin{proof}
	令 $\phi(\boldsymbol{x})$ 为带有周期边界条件的任意函数.由于分数阶拉普拉斯算子是线性算子,根据变分导数的定义,有
\begin{equation}
\begin{aligned}
...
不幸的是,此格式并不能保证质量守恒, 这可以在数值算例中被反映出来.
...
FPAVF-P格式\eqref{eq_PAVF:98}-\eqref{eq_PAVF:101}在离散意义上具有以下能量和质量守恒特性:
...
\end{align}
\newpage
\noindent 将\eqref{eq_PAVF:11221}代入\eqref{eq_PAVF:11147},得到
\begin{align}
...
类似地,通过将\eqref{eq_PAVF:98}-\eqref{eq_PAVF:99}分别与$\delta_t P^{m}$和$\delta_t Q^{m}$作离散内积并求和, 可以得到离散能量守恒律
...
% 其中
% $$\left\|U_{N}^{M}-U_{N}^{2 M}\right\|_{\infty}=\left\|U\left(\frac{T}{M}, \frac{L}{N}\right)-U\left(\frac{T}{2 M}, \frac{L}{N}\right)\right\|_{\infty},\left\|U_{N}^{M}-U_{2 N}^{M}\right\|_{\infty}=\left\|U\left(\frac{T}{M}, \frac{L}{N}\right)-U\left(\frac{T}{M}, \frac{L}{2 N}\right)\right\|_{\infty},$$
% 并且在两个连续的时间步长$\tau$和$\tau / 2$以及两个连续的多项式阶次$N$和$2N$上
计算时间方向和空间方向的收敛阶
\begin{equation}
\text { order }= \left\{
\begin{aligned}
...
\begin{example}\label{exp_PAVF:2}
	考虑一维非线性分数阶波动方程 \eqref{eq_SAVRRK:1}-\eqref{eq_SAVRRK:3}:
...
\begin{figure}[H]
	...
	更准确地说,表 \ref{tab_PAVF:1} 到表 \ref{tab_PAVF:4} 显示了不同 $\alpha$ 值下不同时间 $t=t_n$ 时的离散能量 $H^n$ 和离散质量 $G^n$ 的值,这些值是通过取 $N=512$ 和 $\tau=0.01$ 获得的.从表 \ref{tab_PAVF:1} 可以看出,提出的四种格式均保持了原始能量,而 SAV 格式和三层线性隐式差分格式仅保持了修正后的能量.类似地,从表 \ref{tab_PAVF:2} 到表 \ref{tab_PAVF:4},观察到 FPAVF-P 格式收敛到原始质量,其他方法性能较差,而三层线性隐式差分格式仅保持了修正后的质量.
...
$t$   &FAVF   &FPAVF   &FPAVF-C   &FPAVF-P\\
  ...
  最后,分别在图 \ref{fig_PAVF:13}-图\ref{fig_PAVF:16} 中展示了 $\alpha=1.3,1.6,1.99,2$ 时波的演化过程(取$N=128$ 、 $\tau=0.01$).
...
在本章中,首先重构分数非线性薛定谔波动方程为一个等价的哈密顿系统.
通过将分区平均向量场加方法与傅里叶拟谱方法结合,基于得到的哈密顿系统构建了一个保结构格式.
理论分析和数值结果表明,所提出的方法能够有效地保持原始能量和原始质量.
