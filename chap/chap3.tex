...
然而,众所周知,只有特定的隐式RK方法可能是辛的或代数上稳定的,而没有辛的或代数稳定的显式RK方法.
...
\begin{example}\label{exp_SAVRRK:1}
	...
	不失一般性,设 $\alpha=1.5$ 、 $T=1$. %,以确认显式SAV-RRK方法\eqref{eq_SAVRRK:4-3}的精度.
...
\begin{figure}[H]
	...
	\begin{example}\label{exp_SAVRRK:2}
		...
		% 为了再次确认理论结果在2维情况下的适用性,使用例 \ref{exp_SAVRRK:1} 中相同的参数($\alpha=1.5$ 和 $T=1$).
...
% 本章首先针对具有周期性边界条件的二维非线性分数阶薛定谔波方程,通过标量辅助变量方法将其转化为一个等效系统,其能量被重新表述为三个二次项和的形式.
