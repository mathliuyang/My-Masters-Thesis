...
\newtheorem{remark}{注记}[section] % 备注环境继承定理的编号
\newtheorem{myexample}{算例}[section] % 算例环境继承定理的编号
...
\begin{frame}\frametitle{显式SAV-RRK方法的截断误差}
	\noindent 数值解$\bm{y}_\gamma^{m+1}$在$\hat{t}_{m+1}$处可以从如下两种角度进行解释\cite{ketchesonRelaxationRungeKutta2019}%即增量方向技术(IDT)和松弛技术(RT).
	\begin{itemize}
...
% 	本章首先考虑了具有周期性边界条件的二维非线性分数阶薛定谔波方程,并通过标量辅助变量方法将其转化为一个等价系统,其中能量被重新表述为三个二次项的和.
...
\begin{frame}{在校期间科研成果}\footnotesize
	...
	\bibitem{ref1} {\textbf{\color{purple}Liu Y}}, Ran M, Zhang L, Hamiltonian-preserving schemes for the two-dimensional fractional nonlinear Schrödinger wave equations[J]. Comput. Math. Appl, 2023, 150: 54–69.
	\bibitem{ref2} {\textbf{\color{purple}Liu Y}}, Ran M, Arbitrarily high-order explicit energy-conserving methods for the generalized nonlinear fractional Schrödinger wave equations[J]. Math. Comput. Simulation, 2023, 216: 126-144.
	\bibitem{ref3} Tian Z, Ran M, {\textbf{\color{purple}Liu Y}}, Higher-order energy-preserving difference scheme for the fourth-order nonlinear strain wave equation, Comput[J]. Math. Appl, 2023, 135: 124–133.
	\bibitem{ref4} Zhang X, Ran M, {\textbf{\color{purple}Liu Y}}, Zhang L, A high-order structure-preserving difference scheme for generalized fractional Schrödinger equation with wave operator[J]. Math. Comput. Simulation, 2023, 210: 532–546.
	\bibitem{ref5} Feng Z, Ran M, {\textbf{\color{purple}Liu Y}}, An efficient difference scheme for the non-Fickian time-fractional diffusion equations with variable coefficient[J]. Appl. Math. Lett, 2023, 121: 107489.
	\bibitem{ref6} 谭凤, 冉茂华, {\textbf{\color{purple}刘洋}}. 二维分数阶强耦合Schr{\"o}dinger方程的保结构方法[J]. 四川师范大学学报(自然科学版).(已录用)
	\end{thebibliography}
\end{frame}
