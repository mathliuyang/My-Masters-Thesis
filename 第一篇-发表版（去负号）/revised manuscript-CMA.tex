...
%Due to the memory and genetnic property of fractional operators, fractional differential equations  \cite{liuNovelTechniquesParameter2011} are more suitable to describe or simulate a variety of scientific and engineering problems with memory and hereditary properties than integer order differential equations, thus they are frequently implemented in many scientific fields, such as physics, hydrology and finance  \cite{zaslavskyChaosFractionalKinetics2002} .
...
%\begin{align}\label{eq_3}
%u(\boldsymbol{x}+2\boldsymbol{L}, t)=u(\boldsymbol{x}, t), \quad t \in[0, T]
%\end{align}
where $\mathrm{i}=\sqrt{-1}, 1<\alpha \leq 2, \boldsymbol{x}\in \Omega \subset \mathbb{R}^d ( d=1,2), \kappa$ and $\beta>0$ are both real constants, $u(\boldsymbol{x}, t)$ is unknown complex valued function, $u_{0}(\boldsymbol{x})$ and $u_{1}(\boldsymbol{x})$ are known smooth functions. %, $|\cdot|$ denotes the modulus of complex number.
%and $2\boldsymbol{L}$ is the period. For $1 \mathrm{D}$ case, we set $\Omega=\left[-L_1, L_1\right]$ with periodic boundary conditions, and denote $\boldsymbol{x}=x, \boldsymbol{L}=L_1$.
%For $2 \mathrm{D}$ case, we write
%$\Omega$ is  a bounded domain in $\mathcal{R}^2$.
%, and denote $\boldsymbol{x}=(x, y), \boldsymbol{L}=\left(L_1, L_2\right)$.
The fractional Laplacian can be given by the Fourier transform as
%%\begin{align}\label{eq_4}
%%-(-\Delta)^{\frac{\alpha}{2}} u(\boldsymbol{x},t)=-\mathcal{F}^{-1}\left[|\boldsymbol{\sigma}|^{\alpha} \mathcal{F}(u(\boldsymbol{\sigma},t))\right],
%%\end{align}
%%where $|\boldsymbol{\sigma}|^{\alpha}$ is a symbol, $\mathcal{F}$ is the Fourier transform and $\mathcal{F}^{-1}$ denotes its inverse,
%%$\boldsymbol{\sigma}=(\sigma_1,\sigma_2)$, see  \cite{caffarelliExtensionProblemRelated2007}.
% \begin{align}\label{eq_4}
...
%We develop a numerical method to solve the initial-periodic boundary value problem \eqref{eq_1}-\eqref{eq_3} in a finite domain $\Omega \times(0, T)$.
...
%Around the differential models with Hamiltonian structure, researchers have proposed many structure-preserving algorithms in recent years.
...
%When $\alpha=2$, Eq.\eqref{eq_108} simplifies into the classical nonlinear Schr{\"o}dinger wave equation.
